...
export default function TrackPage() {
  ...
  async function handleSearch(e: React.FormEvent) {
    e.preventDefault()
    if (!serial.trim() && !imei.trim() && !jobNo.trim()) {
      setError('Please enter at least one field.'); return
    }
  ...
  // Only stages that have been logged in history OR the current active stage
  const visibleStages = stages.filter((stage, idx) => {
    const hasHistory = job?.stage_history.find(h => h.stage === stage.key)
    const isActive = idx === currentIdx
    return hasHistory || isActive
  })

  return (
    <>
      <Head>
        <title>Warranty Tracker — Idealz Lanka</title>
      ...
      <div className="min-h-screen" style={{ background: 'linear-gradient(160deg, #0A2240 0%, #1A3A5C 50%, #0A2240 100%)' }}>
        <header className="px-6 py-5 flex items-center justify-between max-w-5xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white font-semibold text-sm">Idealz Lanka</div>
              <div className="text-blue-200 text-xs">Warranty Tracker</div>
            </div>
          </div>
          ...
          <form onSubmit={handleSearch} className="card p-6 text-left shadow-2xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input className="input mono" value={serial} onChange={e => setSerial(e.target.value)} placeholder="e.g. F2LXQ0ABHG7H" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">IMEI Number</label>
                <input className="input mono" value={imei} onChange={e => setImei(e.target.value)} placeholder="15-digit IMEI" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Job Number</label>
                <input className="input mono" value={jobNo} onChange={e => setJobNo(e.target.value)} placeholder="e.g. JOB-001" />
              </div>
            </div>
            {error && (
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full">
              {loading ? 'Searching...' : 'Track My Device'}
            </button>
          </form>
        </div>
        ...
        {job && (
          <div className="max-w-2xl mx-auto px-6 pb-20">
            <div className="card shadow-2xl overflow-hidden">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className="text-xs font-medium px-2 py-1 rounded-full" style={{ background: TYPE_COLORS[job.device_type] + '20', color: TYPE_COLORS[job.device_type] }}>
                        {TYPE_LABELS[job.device_type]}
                      </span>
                      <span className={`badge ${isComplete ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                        {isComplete ? '✓ Completed' : '● In Progress'}
                      </span>
                    </div>
                    <h2 className="text-lg font-semibold text-[#0A2240]">{job.model}</h2>
                    {(job.color || job.storage) && (
                      <p className="text-sm text-slate-500">{[job.color, job.storage].filter(Boolean).join(' · ')}</p>
                    )}
                  </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-3">
                  {job.serial_number && (
                    <div className="bg-white rounded-lg px-3 py-2 border border-slate-200">
                      ...
                      <p className="text-xs font-medium mono text-[#0A2240] truncate">{job.serial_number}</p>
                    </div>
                  )}
                  {job.imei && (
                    <div className="bg-white rounded-lg px-3 py-2 border border-slate-200">
                      <p className="text-xs text-slate-400">IMEI</p>
                      <p className="text-xs font-medium mono text-[#0A2240] truncate">{job.imei}</p>
                    </div>
                  )}
                  <div className="bg-white rounded-lg px-3 py-2 border border-slate-200">
              ...
              {/* Timeline — only shows logged stages */}
              <div className="p-6">
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-5">Warranty Journey</p>
                <div className="relative">
                  {visibleStages.map((stage, visibleIdx) => {
                    const realIdx = stages.findIndex(s => s.key === stage.key)
                    ...
                    const isLast = visibleIdx === visibleStages.length - 1

                    return (
                      <div key={stage.key} className="flex gap-4 pb-6 last:pb-0">
                        <div className="flex flex-col items-center">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 ${
                            isDone && !isActive ? 'bg-green-500' :
                            isActive ? 'bg-[#0A2240] ring-4 ring-[#0A2240]/20' :
                            'bg-slate-200'
                          }`}>
                            {isDone && !isActive
                          ...
                          {!isLast && (
                            <div className={`w-0.5 flex-1 mt-1 ${isDone ? 'bg-green-400' : 'bg-slate-200'}`} style={{ minHeight: '20px' }} />
                          )}
                        </div>
                        <div className="flex-1 pb-1">
                          <p className="text-sm font-medium text-[#0A2240]">{stage.label}</p>
                          {historyEntry && (
                          ...
                          {historyEntry?.note && (
                            <div className="mt-1.5 text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                              {historyEntry.note}
                            </div>
                          )}
                          {isActive && !historyEntry?.note && (
                ...
                {!isComplete && (
                  <div className="mt-5 pt-5 border-t border-slate-100">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <p className="text-xs font-medium text-[#0A2240]">{Math.round(((currentIdx + 1) / stages.length) * 100)}%</p>
                    </div>
                    <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div className="h-full rounded-full bg-[#0A2240] transition-all"
                        style={{ width: `${Math.round(((currentIdx + 1) / stages.length) * 100)}%` }} />
                    </div>
                    <p className="text-xs text-slate-400 mt-2 text-center">
                      Your device is being processed — we will update you at each step
                    </p>
                  </div>
                )}
                ...
                {isComplete && (
                  <div className="mt-5 pt-5 border-t border-slate-100 text-center">
                    <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-2">
                      <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5" stroke="#15803D" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </div>
                    <p className="text-sm font-medium text-green-700">Warranty service completed</p>
                    <p className="text-xs text-slate-400 mt-1">Your device has been handed over</p>
                  </div>
                )}
              </div>
        ...
        <footer className="text-center pb-10 text-blue-300 text-xs">
          © {new Date().getFullYear()} Idealz Lanka Pvt Ltd
        </footer>
      </div>
    </>
  )
